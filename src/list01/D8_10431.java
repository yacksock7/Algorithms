 ...
 *  1 초	    256 MB	    10177	5422	4524	53.768%
 *
 *  문제
...
//            String firstLine = "4";
//
//            String[] sources = ("1 900 901 902 903 904 905 906 907 908 909 910 911 912 913 914 915 916 917 918 919\n" +
//                    "2 919 918 917 916 915 914 913 912 911 910 909 908 907 906 905 904 903 902 901 900\n" +
//                    "3 901 902 903 904 905 906 907 908 909 910 911 912 913 914 915 916 917 918 919 900\n" +
//                    "4 918 917 916 915 914 913 912 911 910 909 908 907 906 905 904 903 902 901 900 919").split("\n");// br.readLine();
    ...
    /**
     *  1. source의 문제 번호와 학생번호를 분리한다. (question, students)
     *
     *  2. student를 정렬한다. TODO 정렬방법이 어떤게 있는지 공부해보자.
     *   - index 0부터 length까지 증가하며 students를 순회한다.
     *   - index-1에 해당하는 value가 index에 해당하는 value 크다면 순서를 바꾸고, jumpCount +1을 한다.
     *   - 순서가 변경되었다면, index-1에 해당하는 value가 index에 해당하는 value 크다면 순서를 바꾸고, jumpCount +1을 한다.
     *   - 순서가 변경되었다면, index-2에 해당하는 value가 index-1에 해당하는 value 크다면 순서를 바꾸고, jumpCount +1을 한다.
     *   - 순서가 변경되지 않았거나, index가 0에 다다른 경우에 중단한다.
     *   - 중단되었다면, index에 +1을 한다.
     *
     */
    private static String mySolution(String source) {
        ...
        for (int i = 0; i < students.length; i++) {
            for (int j = 0; i - j > 0; j++) {
                int idx = i - j;
                if (idx != 0 && students[idx] < students[idx - 1]) {
                    int temp = students[idx - 1];
